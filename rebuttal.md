...
复核确认过采样副本和部分 human--AI pair 可跨训练/验证部分；这会影响早停信号的独立性，但不把相关记录带入独立 DualTest，也不改变报告指标的计算。修正后的记录级匹配复核确认，SarcTrain 与 AuthTrain 均未和 DualTest 的人类侧或 AI 侧共享记录。早期报告的七条表面匹配来自匹配程序本身，并非共享的训练—测试记录；因此最终复核未发现训练—测试泄漏。
