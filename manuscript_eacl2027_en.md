...
Detector scores also matter downstream because they can become generation rewards. ViSP, for example, derives PPO rewards from a sarcasm detector (Wang et al., 2026; §2.4), motivating a comparison between detector sarcasm probabilities and LLM-judge sarcasm ratings. A separate question arises for the second head: predicted human-authorship probability is not itself a measure of perceived human-likeness. We therefore compare that probability with source-blinded human-likeness ratings as well (§6). Existing work nevertheless studies sarcasm detection and MGTD separately; research unifying them as a joint multi-task problem remains absent (see §2.2 for the search protocol).

To test these risks rather than assume MGTD is a beneficial auxiliary task, we compare same-configuration joint and sarcasm-only models on bilingual data and build DualTest for source × sarcasm subgroup analysis. The evaluated joint-training procedure sharply increases sarcasm false positives on AI-generated texts prompted to be non-sarcastic. We call this class-conditional negative transfer because the degradation is concentrated in one source × sarcasm subgroup rather than distributed uniformly across DualTest. The direction persists on Qwen3.5-generated test texts, while two alternative representation configurations show lower observed rates than the plain configuration. By combining probes, task-head correlations, and representation interventions, we show that source-related features align with sarcasm decisions and that removing one data-derived high-variance direction changes the subgroup gap. Determining what this direction encodes and why it affects predictions is a concrete target for causal analysis.
...
- **Representation evidence and a target for causal analysis (§5.3.2).** Source information is highly decodable from the sarcasm representation and correlated with sarcasm decisions. Concept erasure and a control based on a data-derived high-variance direction show that removing this direction changes the subgroup errors. Its semantic content and causal role remain concrete targets for further analysis.
...
Training and evaluation comprise five fold-based runs followed by evaluation on DualTest. SarcTrain is pre-split into five folds: run $k$ trains the sarcasm task on four folds, with the held-out fold as the sarcasm validation set providing $\mathcal{L}_{s}^{(e)}$; AuthTrain uses a fixed 90% training / 10% validation split shared by all runs, its validation portion providing $\mathcal{L}_{a}^{(e)}$, so between-run variation is concentrated in changes to the sarcasm training data. Each run is trained from a fresh initialization of the same pretrained base; all architectures and ablations share the fold assignments and seed scheme, and cross-model comparisons are paired by split. The five models are then evaluated on both tasks on DualTest, which enters no training or validation split for the sarcasm or MGTD models. The main tables report the mean ± standard deviation across the five runs; this standard deviation reflects between-run variation from training splits and initialization, while test-set sampling uncertainty is quantified separately through confidence intervals for the final model (Table 3 and Appendix C.7).
...
After training the sarcasm and MGTD models, we use DualTest first to compare encoder backbones and intermediate configurations, then to compare matched dual- and single-head procedures, and finally to divide predictions into four groups defined by source and sarcasm label. Overall metrics below are five-run means ± standard deviations; subgroup metrics are point estimates from separately trained final models, with paired comparisons made between models on the same texts.

### 5.1 Backbone Comparison

We compare dual-head models across four encoder backbones: mBERT (Devlin et al., 2019), XLM-RoBERTa-base (Conneau et al., 2020), and DeBERTa-v3 base and large (He et al., 2023). On the large encoder, we also evaluate the two intermediate-module variants of §4.1. “Sarcasm F1” is the F1 of the sarcastic class (positive class), following the official iSarcasmEval convention, and “MGTD F1” is the F1 of the human class (positive class); both are computed over the full DualTest set, aggregating Chinese and English and both source groups, with sarcasm labels for AI-generated texts constructed from the generation instructions (§3).
...
Table 1 reports the comparison (five-run mean ± standard deviation). Under the evaluated settings, the DeBERTa configurations have higher mean F1 scores than mBERT and XLM-R. RCNN has the highest means, reaching 0.6309 sarcasm F1 and 0.9301 MGTD F1. Across the five runs, the two intermediate variants also have 34–47% lower observed standard deviations than the plain large model. Because the complete configurations differ jointly in backbone identity, parameter scale, and intermediate representation, the comparison does not isolate the effect of any one design choice. For the separately trained final RCNN model, sarcasm F1 is 0.819 in Chinese and 0.429 in English; class prevalence, genre, annotation procedure, and language all contribute to this difference, so it should not be read as a pure language effect. Macro-averaged, human-subset, and per-language results are in Appendix C.

### 5.2 Joint versus Single-Task Procedures
...
Each row compares a joint model with the corresponding single-task control for the task shown. For sarcasm, the matched joint and sarcasm-only models share the encoder, intermediate structure, sarcasm data, folds, and sarcasm-specific hyperparameters. The joint runs additionally receive AuthTrain batches and MGTD updates. Sarcasm-only controls traverse the full sarcasm training fold each epoch, whereas joint models draw sarcasm batches under the 2:1 task-mixing schedule; comparisons remain paired by fold (§4.2). Joint training lowers mean sarcasm F1 by 6.08 pp for plain and 4.43 pp for RCNN, while mean MGTD F1 changes by only +1.67 and +0.49 pp. In all ten comparisons using the same fold assignments—five per configuration—sarcasm F1 is lower for the jointly trained model (−0.8 to −8.6 pp). The consistent degradation is therefore in sarcasm detection rather than in both tasks. RCNN has a smaller joint–sarcasm-only gap, motivating capacity- and compute-matched tests of which RCNN components contribute to this difference.
...
#### 5.3.1 Three Configurations and Sarcasm-Only Controls

We compare five detectors on the same DualTest set. The three joint configurations show whether the subgroup error changes with the intermediate structure. The plain and RCNN sarcasm-only models provide same-configuration controls. Within each pair, both models use the same all-human sarcasm data; only the joint model additionally undergoes MGTD training. Their difference therefore measures the complete joint-training procedure. Table 3 reports sarcasm predictions; corresponding MGTD source-class metrics are in Appendix C.
...
The plain joint model predicts sarcasm for 86.3% of AI × non-sarcastic instances. The rates are 40.9% for the joint interaction-projection configuration and 34.1% for the joint RCNN configuration, while both matched sarcasm-only controls give 19.3%. Among the same texts, 1,021 are false positives only for the plain joint model, whereas 7 are false positives only for its matched sarcasm-only control ($p < 0.001$). The large increase therefore appears when MGTD training is added. The plain and RCNN models provide matched contrasts. The interaction-projection configuration changes architecture and optimization factors together and is therefore included only as an additional comparison.

The MGTD head does not show an analogous extreme error: its false-positive rate on this subset is 6.1% for the plain model and 6.6% for RCNN. Within AI-generated texts, sarcasm AUC remains 0.82–0.88 across models, so the relative ranking of sarcastic and non-sarcastic AI-generated texts is largely preserved. For the plain pair, the median sarcasm probability is 0.767 (IQR 0.635–0.839) under joint training versus 0.204 under sarcasm-only training, placing much of the joint distribution above the fixed 0.5 threshold. This pattern is consistent with a broad upward score shift rather than lost within-source ranking.

The direction holds in both languages: joint-minus-sarcasm-only differences on AI-generated texts prompted to be non-sarcastic are +63.5 pp (English) and +79.4 pp (Chinese) for plain, and +16.9 and +7.7 pp for RCNN. Language remains coupled with corpus, genre, and annotation setting.

#### 5.3.2 Representation-Level Diagnostics

We ask whether source is merely linearly available, whether source-related scores align with sarcasm decisions, and whether representation interventions alter the subgroup gap. We analyze frozen representations from the separately trained final plain joint model underlying the 86.3% false-positive rate in Table 3. Five-fold cross-validated logistic-regression probes trained on the frozen representations predict human versus AI source at AUC 0.986 for joint and 0.974 for the matched sarcasm-only model, showing that source decodability alone does not explain the gap. In the target subgroup, the MGTD-head AI logit correlates with the sarcasm logit ($\rho=0.519$). Although the MGTD and sarcasm head directions---each defined as the difference between its two class-weight vectors---are nearly orthogonal under Euclidean cosine ($\cos\approx-0.03$), their covariance-normalized alignment is 0.49; the corresponding probe-weight--sarcasm alignment is 0.41. Covariance normalization accounts for variation in the observed representations, whereas ordinary cosine compares classifier directions only in Euclidean parameter space. These results are correlational.

We fit interventions on one half of the frozen representations and evaluate them on the other, where the baseline AI/human non-sarcastic false-positive rates are 84.8%/32.8% and source AUC is 0.982. LEACE, a linear concept-erasure method applied without retraining the sarcasm head, reduces linearly decodable source information: a source probe retrained after erasure gives AUC 0.589, while the rates change to 68.7%/49.5%. Their unweighted mean is nearly unchanged (58.8% to 59.1%), indicating error reallocation rather than overall improvement. Simpler source-direction and INLP removals produce no comparable reduction. The AI–human gap therefore decreases from 52.0 pp to 19.1 pp under LEACE.

Separately, srcORTH-PC1 tests whether error allocation can change without directly removing the estimated linear source direction: it is the maximum-variance direction after projecting that direction out. Removing srcORTH-PC1 changes the rates to 66.7%/54.1% and reduces the gap to 12.6 pp, while source AUC remains 0.982; random-direction controls produce no comparable change. Thus, reducing measured linear source decodability is not necessary for altering subgroup error allocation. We treat srcORTH-PC1 as behaviorally relevant but semantically uncharacterized, not as an identified source feature.

#### 5.3.3 Evaluation on Qwen3.5-Generated Texts without Detector Retraining

We keep the trained detectors fixed and compare AI test subsets generated by DeepSeek-R1-0528-Qwen3-8B and Qwen3.5-9B (Ollama tag `qwen3.5:9b`). After quality control, the DeepSeek-generated and Qwen3.5-generated non-sarcastic subsets contain 1,513 and 1,550 outputs. On Qwen3.5 outputs, the plain joint and sarcasm-only false-positive rates are 91.3% and 7.9%, an observed gap of 83.4 pp compared with 67.0 pp on DeepSeek. The joint RCNN rate is 22.1%, versus 10.6% for its matched sarcasm-only control; the joint interaction-projection rate is 29.5%. The higher joint-versus-sarcasm-only rate therefore persists on another generator, while absolute rates and gaps differ.
...
**C.7 Uncertainty from test instances: confidence intervals, paired tests, and subset F1.** For each separately trained final model, this section quantifies uncertainty from the test instances with Wilson 95% intervals for quadrant rates and 95% percentile intervals from 10,000 instance-level bootstrap samples for F1. These intervals do not include uncertainty from training, generator sampling, corpus construction, or human–AI pairing. They complement the between-run standard deviations in Tables 1 and 2. The final models show the same directional joint-versus-sarcasm-only gap on the Qwen3.5-generated texts, although its magnitude differs.
...
RCNN MGTD F1 is 0.938 [0.931, 0.945] on the full set, 0.961 [0.952, 0.969] in Chinese, and 0.923 [0.912, 0.933] in English. Because the human subset contains no AI-labeled negatives, a source-class F1 is not separately reported for this subset; Table C.2 reports human recall directly. RCNN sarcasm F1 is higher in Chinese than English (0.819 vs. 0.429), but the contrast is confounded by prevalence, genre, annotation procedure, and language: sarcasm prevalence is 63.8% in Chinese but 14.2% in English, and the human subsets come from different benchmarks. These positive-class F1 values are not directly comparable as a pure language contrast. RCNN overall sarcasm F1 is 0.663 versus 0.629 on human-authored texts alone because the overall value also includes AI-generated texts. On the human subset, joint-versus-sarcasm-only differences are within 0.03 and vary in direction; the visible negative-transfer loss is concentrated in the quadrants containing AI-generated texts.

False-positive rates use a threshold of 0.5. Instance-level scores show a broad difference around that threshold: for the plain dual-head model on the AI × non-sarcastic subset, the median sarcasm probability is 0.767 (interquartile range 0.635–0.839), placing the middle half of the distribution above 0.5. The single-head medians are 0.204 (plain) / 0.069 (RCNN). Separately, similar AUC values within AI-generated texts (0.82–0.88) show that their internal ordering is largely preserved.
...
Bold marks the two diagnostic interventions (LEACE and srcORTH-PC1), not best performance. Interpretation: single-direction and low-dimensional INLP erasure produce little change. LEACE reduces measured linear source decodability and the false-positive-rate difference between source groups from 52 to 19, while the ten sampled random-direction controls show no comparable change. Erasing a high-variance direction orthogonal to the estimated linear source direction changes the AI-side rate to 66.7 and the difference between source groups to 12.6 while leaving measured linear source decodability unchanged. Together, these controls show that removing this direction changes how errors are distributed between source groups and make its semantic content and causal role concrete targets for further analysis. Under LEACE, the unweighted mean false-positive rate across the two non-sarcastic source groups is almost unchanged (58.8 → 59.1), showing that the affected directions shape how errors are allocated between AI and human texts.
