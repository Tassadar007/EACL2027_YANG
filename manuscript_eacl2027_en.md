...
We use five fold-matched runs. In run $k$, four SarcTrain folds train the sarcasm task and the fifth provides validation; all runs share a fixed AuthTrain 90/10 split, whose approximately 2,000-example holdout provides a common MGTD validation set. Each run reloads its configuration's pretrained encoder and newly initializes the remaining modules and heads; all configurations use the same folds and seed schedule. DualTest is held out until evaluation. We report mean ± standard deviation across runs to summarize variation from the sarcasm fold and initialization; for subgroup rates from separately trained final models, Wilson intervals summarize uncertainty over DualTest examples (Table 3 and Appendix C.7).
...
We compare backbones and joint- versus single-task procedures on DualTest using five-run aggregates, then use separately trained final models for source × sarcasm analysis, representation diagnostics, paired tests, and Wilson intervals.

### 5.1 Backbone Comparison

We compare dual-head mBERT (Devlin et al., 2019), XLM-RoBERTa-base (Conneau et al., 2020), and DeBERTa-v3 base and large (He et al., 2023) under one protocol; for DeBERTa-v3-large, we also evaluate the interaction-projection and RCNN configurations. On DualTest, we report positive-class F1 for sarcastic texts and human-authored texts in the two tasks.
...
Under this protocol, the DeBERTa configurations yield higher mean F1 than mBERT and XLM-R, and RCNN achieves the highest means: 0.6309 for sarcasm and 0.9301 for MGTD (Table 1). These system-level comparisons jointly reflect backbone family, model scale, representation design, and optimization. On this empirical basis, the subsequent analyses use DeBERTa-v3-large, with the plain shared encoder as the reference and interaction-projection and RCNN as alternative representation pathways. Macro-averaged, human-subset, and language-stratified results are in Appendix C.
...
Each sarcasm row compares matched joint and sarcasm-only procedures under the controls in §4.2; the contrast concerns the complete joint-training procedure, including its scheduling and shared updates, rather than MGTD supervision alone. Across the ten fold-matched sarcasm comparisons—five per configuration—joint training lowers sarcasm F1 in every run, by 6.08 pp on average for plain and 4.43 pp for RCNN. MGTD does not show the corresponding consistent decline: its mean F1 changes by +1.67 and +0.49 pp. The joint-versus-single-task sarcasm gap is smaller for RCNN. We next locate the aggregate degradation across source × sarcasm subgroups.

### 5.3 Source × Sarcasm Subgroup Analysis

To determine whether the aggregate degradation is uniform, we evaluate the four source × sarcasm subgroups separately. Each contains one sarcasm reference class, so we report recall for sarcastic groups and false-positive rate for non-sarcastic groups; human texts retain dataset annotations, whereas AI-text labels denote prompt-specified style.

#### 5.3.1 Three Configurations and Sarcasm-Only Controls

Plain and RCNN provide same-configuration matched comparisons: within each pair, both models use the same all-human sarcasm data, while only the joint model also receives MGTD training. The interaction-projection configuration lacks a matched sarcasm-only control and is therefore descriptive. Table 3 reports sarcasm predictions; corresponding MGTD results are in Appendix C.
...
Brackets in the final column give Wilson 95% confidence intervals over test instances. Intervals for all quadrants, paired tests, and score distributions are provided in Appendix C.7.

On AI-generated texts prompted to be non-sarcastic, the plain joint and sarcasm-only false-positive rates are 86.3% and 19.3%, a 67.0-pp difference; the matched RCNN rates are 34.1% and 19.3%, a 14.8-pp difference. Exact paired McNemar tests give $p<.001$ for both comparisons (Appendix C.8).

Within AI-generated texts, sarcasm AUC remains 0.82–0.88 across the models in Table 3, so their ranking of sarcastic versus non-sarcastic texts is largely preserved. For the plain pair, median sarcasm probability is 0.767 under joint training and 0.204 under sarcasm-only training. Together with the false-positive rates, this is consistent with an upward score shift rather than a collapse in within-source ranking.

For plain, the non-sarcastic false-positive-rate change is 4.3 pp on human texts but 67.0 pp on AI-generated texts (differences use unrounded rates). The direction holds in both languages: joint-minus-sarcasm-only differences on AI-generated non-sarcastic texts are +63.5 pp (English) and +79.4 pp (Chinese) for plain, and +16.9 and +7.7 pp for RCNN. Language remains coupled with corpus, genre, and annotation setting. The degradation is therefore concentrated in one source × sarcasm subgroup rather than distributed uniformly across DualTest.

#### 5.3.2 Representation-Level Diagnostics

**Scope.** We analyze frozen pooled representations from the final plain joint model underlying the 86.3% rate in Table 3; the matched sarcasm-only model is used only for the source-probe comparison. We test source decodability, alignment with sarcasm decisions, and the effect of representation interventions. The results characterize this single trained model, not all configurations or seeds.

**Decodability and alignment.** Five-fold logistic-regression source probes give AUC 0.986 for the joint representations and 0.974 for the matched sarcasm-only representations. Both are high despite sharply different subgroup gaps, so decodability alone cannot explain the joint-model error. On AI-generated non-sarcastic texts, the MGTD AI logit correlates with the sarcasm logit ($\rho=0.519$). Head directions are differences between their class-weight vectors, and the probe direction is the logistic-regression weight vector. The MGTD–sarcasm cosine is approximately $-0.03$ in Euclidean geometry; covariance-aware alignment, which evaluates the directions under the covariance structure of the observed representations, is 0.49 between the task heads and 0.41 between the source probe and sarcasm head. Because the covariance uses pooled unlabeled DualTest representations, the alignment is transductive; all these results are correlational.

**Intervention protocol.** We split DualTest examples into disjoint fitting and evaluation halves. Directions and transformations are estimated only on the fitting half and applied to the evaluation half, while the encoder and sarcasm head remain frozen; residual source AUC is measured by a newly cross-validated probe within the evaluation half. The unmodified false-positive rates are 84.8% for AI-generated non-sarcastic texts and 32.8% for human-authored non-sarcastic texts (gap 52.0 pp), rather than 86.3% on full DualTest; source AUC is 0.982.

**Source-information erasure.** LEACE reduces source AUC to 0.589 and changes the AI and human false-positive rates to 68.7% and 49.5%, respectively (gap 19.1 pp). Their unweighted mean is nearly unchanged (58.8% to 59.1%), indicating error reallocation rather than average improvement. Direct removal of the MGTD or probe direction, INLP, and random-direction controls produce no comparable gap reduction.

**High-variance sensitivity test.** We obtain srcORTH-PC1 by projecting the fitting representations off an estimated linear source direction and extracting the remaining maximum-variance direction. Removing its projection changes the AI and human evaluation rates to 66.7% and 54.1%, respectively (gap 12.6 pp), while source AUC remains 0.982. Thus, reducing measured linear source decodability is not necessary for changing error allocation. Orthogonality to one estimated direction does not exclude other source information, so srcORTH-PC1 is behaviorally relevant but semantically uncharacterized, not an identified source feature or a complete causal explanation.

#### 5.3.3 Evaluation on Qwen3.5-Generated Texts without Detector Retraining

Using the same language × style templates, we evaluate the unchanged detectors on Qwen3.5-9B outputs (Ollama tag `qwen3.5:9b`). Without retraining, the false-positive rates on Qwen3.5-generated non-sarcastic texts are 91.3% and 7.9% for the plain joint and sarcasm-only models (gap 83.4 pp, versus 67.0 pp on DeepSeek), and 22.1% and 10.6% for the matched RCNN models. The higher joint-training rate therefore persists on another generator, while magnitudes differ.
