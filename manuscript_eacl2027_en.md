...
Sarcasm is a prototypical form of non-literal expression whose intended meaning may oppose its surface wording. As a characteristically human form of expression, it relies on implicit intent, contrast, context, and background knowledge, making automatic detection difficult; multilingual variation and inconsistent annotation compound the problem.

LLM-based data augmentation offers one potential response. It is useful only if generated texts both preserve sarcastic intent and resemble human writing; otherwise, source-specific artifacts may become shortcuts. Sarcasm and authorship must therefore be considered together: human-authored and AI-generated texts can each be sarcastic or non-sarcastic. In deployment, “is this sarcastic?” and “was this written by a human?” may consequently need to be answered for the same input.

Detector scores also matter downstream because they can become generation rewards. ViSP, for example, derives PPO rewards from a sarcasm detector (Wang et al., 2026; §2.4). Yet useful generated sarcasm must satisfy two distinct objectives: conveying sarcastic intent and being perceived as human-like. A dual-head detector may appear to provide corresponding signals through sarcasm and authorship probabilities, but authorship probability is not itself a measure of human-likeness. We therefore test whether these detector scores track source-blinded LLM ratings of sarcasm and perceived human-likeness (§6). Existing work nevertheless studies sarcasm detection and MGTD separately; research unifying them as a joint multi-task problem remains absent (see §2.2 for the search protocol).

To test these risks rather than assume MGTD is a beneficial auxiliary task, we compare same-architecture dual- and single-head models on bilingual data and build DualTest for source × sarcasm subgroup analysis. The evaluated joint-training procedure sharply increases sarcasm false positives on AI text prompted to be non-sarcastic; we call this class-conditional negative transfer. The direction persists when the AI test texts are regenerated with Qwen3.5-9B, while two intermediate configurations show lower rates than the plain configuration. By combining probes, task-head correlations, and representation interventions, we identify a non-random high-variance direction to which the subgroup gap is sensitive. These analyses support a source-correlated shortcut hypothesis and make the semantic content of the effective direction a concrete target for causal analysis.
...
- **Behavioral diagnosis of class-conditional negative transfer (§5).** The plain dual-head model misclassifies 86.3% of AI-generated non-sarcastic texts as sarcastic; a single-head model with the same sarcasm supervision misclassifies only 19.3%. The increase is induced by the complete joint-training procedure, its direction persists on Qwen3.5-generated test texts, and two intermediate configurations reduce its observed severity.
- **Representation-level characterization and causal targets (§5.3.2).** Source information is highly decodable from the sarcasm representation and correlated with sarcasm decisions. Concept erasure and an orthogonal high-variance control show prediction sensitivity to a non-random high-variance direction. The results support a source-correlated shortcut hypothesis and identify the direction's semantic content and source-specific causal role as concrete targets for further analysis.
...
With the sarcasm and MGTD models fixed, we use DualTest first to compare encoder backbones and intermediate configurations, then to compare matched dual- and single-head procedures, and finally to divide predictions into four groups defined by source and sarcasm label. Overall metrics below are five-run means ± standard deviations; subgroup metrics are fixed-final-model point estimates, with paired comparisons made between models on the same texts.

### 5.1 Backbone Comparison

We compare dual-head models across four encoder backbones: mBERT (Devlin et al., 2019), XLM-RoBERTa-base (Conneau et al., 2020), and DeBERTa-v3 base and large (He et al., 2023). On the large encoder, we also evaluate the two intermediate-module variants of §4.1. “Sarcasm F1” is the F1 of the sarcastic class (positive class), following the official iSarcasmEval convention, and “MGTD F1” is the F1 of the human class (positive class); both are computed over the full DualTest set, aggregating Chinese and English and both source sides, with AI-side sarcasm labels constructed from the generation instructions (§3).
...
Table 1 reports the comparison (five-run mean ± standard deviation). Under the evaluated settings, the DeBERTa configurations have higher mean F1 scores than mBERT and XLM-R. RCNN has the highest means, reaching 0.6309 sarcasm F1 and 0.9301 MGTD F1. Across the five runs, the two intermediate variants also have 34–47% lower observed standard deviations than the plain large model, providing configuration-level evidence of lower run-to-run variation. The table evaluates complete configurations that jointly differ in backbone identity, parameter scale, and intermediate representation. For the separately trained final RCNN model, sarcasm F1 is 0.819 in Chinese and 0.429 in English; class prevalence, genre, annotation procedure, and language all contribute to this difference, so it should not be read as a pure language effect. Macro-averaged, human-subset, and per-language results are in Appendix C.

### 5.2 Dual-Head versus Single-Head Comparison

**Table 2: Dual-head vs. single-head models (mean ± standard deviation over five fold-based runs)**
...
Within each architecture, the dual- and single-head models share the encoder, intermediate structure, sarcasm data, folds, and sarcasm-specific hyperparameters. The dual-head runs additionally receive AuthTrain batches and MGTD updates. Single-head controls traverse the full sarcasm training fold each epoch, whereas dual-head models draw sarcasm batches under the 2:1 task-mixing schedule; comparisons remain paired by fold (§4.2). Joint training lowers mean sarcasm F1 by 6.08 pp for the plain model and 4.43 pp for RCNN, while mean MGTD F1 changes by only +1.67 and +0.49 pp. In all ten split-matched comparisons—five per architecture—sarcasm F1 is lower for the jointly trained model (−0.8 to −8.6 pp). The consistent degradation is therefore in sarcasm detection rather than in both tasks. RCNN has a smaller dual–single gap, motivating capacity- and compute-matched tests of which RCNN components contribute to this difference.

### 5.3 Source × Sarcasm Subgroup Analysis

An overall F1 score can hide a failure concentrated in one type of text. We divide DualTest into four groups defined by source and sarcasm label and examine their predictions separately. Human labels are dataset annotations, whereas AI labels record prompted conditions. Because each group contains only one reference class, we report recall for sarcastic texts and false-positive rate for non-sarcastic texts.
...
The plain dual-head model predicts sarcasm for 86.3% of AI × non-sarcastic instances. The rates are 40.9% for interaction projection and 34.1% for RCNN, while both same-architecture single-head controls give 19.3%. Among the same texts, 1,021 are false positives only for the plain dual-head model, whereas 7 are false positives only for its single-head control ($p < 0.001$). The large increase therefore appears when MGTD training is added. RCNN shows the same direction with a smaller gap. The matched contrasts are provided by the plain and RCNN models, while interaction projection adds complementary configuration-level evidence.

The MGTD head does not show an analogous extreme error: its false-positive rate on this subset is 6.1% for the plain model and 6.6% for RCNN. Within AI-generated texts, sarcasm AUC remains 0.82–0.88 across models, so the relative ranking of sarcastic and non-sarcastic AI texts is largely preserved. Joint training instead shifts predicted sarcasm probabilities upward, causing many texts prompted to be non-sarcastic to cross the fixed 0.5 threshold. The observed failure is therefore an AI-associated score increase on this subgroup, not a complete loss of within-AI ranking ability. We refer to this concrete pattern as class-conditional negative transfer.

#### 5.3.2 Mechanism Hypothesis and Representation-Level Diagnostics

We next ask whether source information in the shared representation is connected to the higher sarcasm scores. The shared encoder and intermediate module receive MGTD updates, while the sarcasm head is trained with the all-human SarcTrain. This training structure motivates the source-correlated shortcut hypothesis tested below.

We test it on the frozen plain dual-head model; Appendix E records the full protocol and self-checks. A linear probe predicts source from the dual-head representation at AUC 0.986. The matched single-head representation also reaches 0.974, establishing source decodability as a background property shared by both models. In the dual-head model, texts with a higher MGTD-head AI logit also tend to receive a higher sarcasm logit on AI × non-sarcastic text (Spearman 0.52). After covariance normalization, the source directions estimated from the MGTD head and the probe are moderately aligned with the sarcasm direction ($|\cos| \approx 0.41$–0.49). Together, the logit correlation and directional alignment provide correlational evidence linking source-related structure to the affected sarcasm decision.

We then alter the frozen representations and recompute predictions. LEACE reduces linearly decodable source information from AUC 0.982 to 0.589 and changes the AI/human false-positive rates from 84.8%/32.8% to 68.7%/49.5%. The nearly unchanged mean across the two groups shows that LEACE specifically reallocates errors between source groups, revealing that the affected directions shape source-conditioned error allocation. A key control removes a high-variance direction constructed to be orthogonal to the estimated linear source direction. It changes the AI/human rates to 66.7%/54.1% while measured source decodability remains 0.982; ten sampled random directions show no comparable effect. Together, these controls show prediction sensitivity to a non-random high-variance direction. The source-correlated shortcut hypothesis therefore provides a focused mechanism for further analysis, with the semantic content of the effective direction as the next causal target.

#### 5.3.3 Cross-Generator Replication

We keep the trained detectors fixed and compare the DualTest AI texts generated by DeepSeek-R1-0528-Qwen3-8B with replacements generated by Qwen3.5-9B (Ollama tag `qwen3.5:9b`). After quality control, the DeepSeek-generated and Qwen3.5-generated non-sarcastic subsets contain 1,513 and 1,550 outputs. The plain dual-head false-positive rates are 86.3% and 91.3%, while the matched single-head rates are 19.3% and 7.9%, increasing the gap from 67.0 to 83.4 pp. On the Qwen3.5-generated texts, RCNN and interaction projection remain below the plain dual-head rate (22.1% and 29.5%). Qwen3.5 therefore reproduces the direction of the instruction-conditioned dual–single subgroup gap, although its magnitude differs.
...
**C.4 Paired tests between SFT and human references (supplementary).** This test is performed on the Chinese subset of the generation-evaluation set ($n = 180$; GPT-5-nano is missing three instances, leaving 177). These comparisons provide judge- and dimension-specific calibration context: for DeepSeek-V3, SFT is significantly below the reference on human-likeness (−0.161, $p = 0.0025$), whereas for GPT-5-nano, SFT is significantly above the reference on sarcasm (+0.186, $p = 0.0114$). The remaining four comparisons are statistically indistinguishable at the reported threshold, showing that proximity to the human reference depends on the judge and evaluated dimension.
...
**C.7 Conditional test-instance uncertainty: confidence intervals, paired tests, and subset F1.** Conditional on each fixed final model, this section quantifies test-instance uncertainty with Wilson 95% intervals for quadrant rates and 95% percentile intervals from 10,000 instance-level bootstrap samples for F1. These intervals do not include uncertainty from training, generator sampling, corpus construction, or human–AI pairing. They complement the between-run standard deviations in Tables 1 and 2. The fixed final-model configurations show the same directional dual–single gap on the Qwen3.5-generated AI texts, although its magnitude differs.
...
RCNN MGTD F1 is 0.938 [0.931, 0.945] on the full set, 0.961 [0.952, 0.969] in Chinese, and 0.923 [0.912, 0.933] in English. Because the human subset contains no AI-labeled negatives, a source-class F1 is not separately reported for this subset; Table C.2 reports human recall directly. RCNN sarcasm F1 is higher in Chinese than English (0.819 vs. 0.429), but the contrast is confounded by prevalence, genre, annotation procedure, and language: sarcasm prevalence is 63.8% in Chinese but 14.2% in English, and the human subsets come from different benchmarks. These positive-class F1 values are not directly comparable as a pure language contrast. RCNN overall sarcasm F1 is 0.663 versus 0.629 on human texts alone because the overall value also includes AI texts. On the human subset, dual–single differences are within 0.03 and vary in direction; the visible negative-transfer loss is concentrated in the AI-side quadrants.
...
## Appendix D: Illustrative Cases of Detector–LLM Score Misalignment <!-- excluded from page limit; two columns -->

Illustrative cases were manually selected after inspecting the paired RCNN-detector and LLM-score table to show large disagreements; no formal ranking or top-k selection was implemented. Detector scores range from 0 to 1, and LLM scores are the three-evaluator mean on a 1–5 scale; long texts are truncated. Chinese examples retain the original model input and provide an English translation in brackets.

**Assigned high human-authorship probability by the detector but lower perceived human-likeness ratings by the LLMs (MGTD-score disagreement; corresponding to §6).**
...
## Appendix E: Representation-Level Analysis of Source-Conditioned Error Allocation <!-- excluded from page limit; two columns -->
