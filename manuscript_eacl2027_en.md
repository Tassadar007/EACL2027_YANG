...
The MGTD head does not show an analogous extreme error: its false-positive rate on this subset is 6.1% for the plain model and 6.6% for RCNN. Within AI-generated texts, sarcasm AUC remains 0.82–0.88 across models, so the relative ranking of sarcastic and non-sarcastic AI-generated texts is largely preserved. Joint training instead raises predicted sarcasm probabilities on AI-generated texts, causing many texts prompted to be non-sarcastic to cross the fixed 0.5 threshold while their internal ranking remains largely preserved. We refer to this concrete pattern as class-conditional negative transfer.

#### 5.3.2 Representation-Level Diagnostics

We next ask whether source information in the shared representation is connected to the higher sarcasm scores. The shared encoder and intermediate module receive MGTD updates, while the sarcasm head is trained with the all-human SarcTrain. This training structure motivates testing whether source-related features influence the sarcasm decisions examined below.

We test it on the frozen plain joint model; Appendix E records the full protocol and self-checks. A linear probe predicts source from the joint representation at AUC 0.986. The matched sarcasm-only representation also reaches 0.974, establishing source decodability as a background property shared by both models. In the joint model, texts with a higher MGTD-head AI logit also tend to receive a higher sarcasm logit on AI × non-sarcastic text (Spearman 0.52). In the whitened representation space, the source directions estimated from the MGTD head and the probe are moderately aligned with the sarcasm direction ($|\cos| \approx 0.41$–0.49). Together, the logit correlation and directional alignment provide correlational evidence linking source-related structure to the affected sarcasm decision.

We then alter the frozen representations and recompute predictions. LEACE reduces linearly decodable source information from AUC 0.982 to 0.589 and changes the AI/human false-positive rates from 84.8%/32.8% to 68.7%/49.5%. The nearly unchanged mean across the two groups shows that LEACE mainly redistributes errors between source groups. As a second control, we define the source-direction-orthogonal first principal component (srcORTH-PC1) as the maximum-variance direction remaining after projecting out the estimated linear source direction. Removing srcORTH-PC1 changes the AI/human rates to 66.7%/54.1% while measured source decodability remains 0.982; ten sampled random directions show no comparable effect. The preceding correlations and directional alignment associate source-related structure with the affected decisions; the intervention separately shows that subgroup errors are sensitive to srcORTH-PC1, identifying it as a target for further causal analysis.

#### 5.3.3 Evaluation on Qwen3.5-Generated Texts without Detector Retraining

We keep the trained detectors fixed and compare AI test subsets generated by DeepSeek-R1-0528-Qwen3-8B and Qwen3.5-9B (Ollama tag `qwen3.5:9b`). After quality control, the DeepSeek-generated and Qwen3.5-generated non-sarcastic subsets contain 1,513 and 1,550 outputs. The plain joint false-positive rates are 86.3% and 91.3%, while the matched sarcasm-only rates are 19.3% and 7.9%, increasing the gap from 67.0 to 83.4 pp. On the Qwen3.5-generated texts, the joint RCNN and interaction-projection configurations remain below the plain joint rate (22.1% and 29.5%). Thus, the higher false-positive rate of the joint procedure relative to the sarcasm-only procedure also appears on outputs from another generator, although its magnitude differs.
...
False-positive rates use a threshold of 0.5. Instance-level score distributions show that the failure is not a threshold artifact: for the plain dual-head model on the AI × non-sarcastic subset, the median sarcasm probability is 0.767 (interquartile range 0.635–0.839), so the entire distribution shifts rightward across the threshold. The single-head medians are 0.204 (plain) / 0.069 (RCNN). Separately, similar AUC values within AI-generated texts (0.82–0.88) show that their internal ordering is largely preserved.
