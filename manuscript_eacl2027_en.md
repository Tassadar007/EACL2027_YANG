...
We organize the analysis in four stages: system selection, matched joint–single-task comparison, source × sarcasm evaluation across languages and generators, and representation-level diagnosis.

### 5.1 System-Level Comparisons

**Backbone selection.** Under the same protocol, we compare dual-head mBERT (Devlin et al., 2019), XLM-RoBERTa-base (Conneau et al., 2020), and DeBERTa-v3 base and large (He et al., 2023); for DeBERTa-v3-large, we also test interaction-projection and RCNN pathways. DualTest positive-class F1 is reported for both tasks.
...
The DeBERTa configurations have higher mean F1 than mBERT and XLM-R, with RCNN highest on both tasks (0.6309 sarcasm; 0.9301 MGTD). Subsequent analyses use DeBERTa-v3-large, with the plain encoder as the reference and interaction-projection and RCNN as alternatives. Complete results are in Appendix C.
...
**Ranking and score shift.** Within AI-generated texts, sarcasm AUC remains 0.82–0.88. For the plain pair, median sarcasm probability rises from 0.204 to 0.767 under joint training, whose interquartile range lies above the decision threshold. The high FPR therefore reflects an upward score shift rather than collapsed within-AI discrimination.
...
The upward score shift with preserved within-AI ranking motivates analysis of the pooled representation, the single encoder vector supplied to both task heads. We examine the final plain joint model, using its matched sarcasm-only model only for the source-encoding comparison.

**Source decodability.** A linear probe is a classifier trained on frozen representations to test whether an attribute can be recovered without changing the analyzed model (Hewitt and Liang, 2019). A logistic-regression source probe reaches AUC 0.986 on joint-model representations and 0.974 on sarcasm-only representations. Both models therefore encode source strongly, so decodability alone does not explain their different subgroup gaps.

**Association with sarcasm decisions.** Probe recovery does not show that the trained head follows source-related structure. A logit is a class score before conversion to a probability or label; on AI-generated non-sarcastic texts, MGTD AI and sarcasm logits correlate at $\rho=0.519$. A binary head's decision direction is its class-1 minus class-0 weight vector. The task heads are nearly orthogonal under ordinary cosine ($\cos\approx-0.03$), but this ignores where representations vary. Covariance-aware alignment based on Ledoit–Wolf covariance weights directions by observed representation geometry (Ledoit and Wolf, 2004); absolute alignment is 0.49 between task heads and 0.41 between source-probe and sarcasm-head directions. Source-related scores and data-weighted directions therefore track the affected decisions.

**Intervention protocol and direct source directions.** To move beyond association, we remove selected components and reapply the unchanged sarcasm head (Elazar et al., 2021). Task-head directions come from the model; data-derived directions and transformations are fitted on one half of DualTest and evaluated on the other. The source-group gap is AI-generated minus human-authored FPR on non-sarcastic texts, and source AUC is the post-intervention probe AUC. The untouched evaluation-half baseline is a 52.0-pp gap with source AUC 0.982; unlike the earlier 67.0-pp joint–single result, this compares source groups within one model.

We first remove MGTD-head and source-probe directions. Iterative nullspace projection (INLP) repeatedly learns linear source directions and removes their joint span (Ravfogel et al., 2020); random directions and subspaces control for arbitrary dimension removal. These interventions, including up to 16 INLP directions, do not materially narrow the gap (51.9–72.1 pp). Covariance-whitened random directions likewise show no comparable reduction. Direct and fitted linear source directions therefore do not account for the gap alone.

**Covariance-aware and residual geometry.** Linear concept erasure (LEACE) is a closed-form covariance-aware transformation that removes linearly accessible concept information while minimizing other distortion (Belrose et al., 2023). LEACE narrows the gap from 52.0 to 19.1 pp as source AUC falls from 0.982 to 0.589; nearly unchanged mean FPR across source groups indicates error reallocation rather than a change in average error.

We construct an exploratory, study-specific diagnostic, srcORTH-PC1, to test high-variance geometry outside a fitted source subspace. After fitting three INLP source directions, we identify the greatest-variance direction in their orthogonal complement; evaluation removes only this direction from the original representations. The gap narrows to 12.6 pp while source AUC remains 0.982 at the reported precision. Residual source AUC and the FPR gap use different readouts: a newly fitted source probe and the frozen sarcasm head, respectively. Their divergence is consistent with source remaining recoverable from other representation structure while the removed direction shifts the two source groups differently relative to the fixed sarcasm boundary. The result identifies sensitivity to this residual direction; its semantic content remains a target for characterization and targeted causal tests.
...
Bold marks the two diagnostic interventions (LEACE and srcORTH-PC1), not best performance. Interpretation: single-direction and low-dimensional INLP erasure produce little change. LEACE reduces measured linear source decodability and the false-positive-rate difference between source groups from 52 to 19, while the ten sampled random-direction controls show no comparable change. Erasing a high-variance direction orthogonal to the estimated linear source direction changes the AI-side rate to 66.7 and the difference between source groups to 12.6 while leaving measured linear source decodability unchanged. Residual source AUC comes from a newly fitted probe, whereas the FPR gap comes from the frozen sarcasm head; their divergence is consistent with source remaining recoverable from other structure while the removed direction shifts the groups differently relative to the fixed sarcasm boundary. The result identifies sensitivity to this residual direction and makes its semantic content a target for further analysis. Under LEACE, the unweighted mean false-positive rate across the two non-sarcastic source groups is almost unchanged (58.8 → 59.1), showing that the affected directions shape how errors are allocated between AI and human texts.
