...
We analyze frozen representations from the separately trained final plain joint model underlying the 86.3% false-positive rate in Table 3. Five-fold source probes achieve AUCs of 0.986 and 0.974 on the joint and matched sarcasm-only representations, showing that source decodability alone does not explain the gap. In the target subgroup, the MGTD-head AI logit correlates with the sarcasm logit ($\rho=0.519$). Although the MGTD and sarcasm head directions---each defined as the difference between its two class-weight vectors---are nearly orthogonal under Euclidean cosine ($\cos\approx-0.03$), their covariance-normalized alignment is 0.49; the corresponding probe-weight--sarcasm alignment is 0.41. These results are correlational.

We fit interventions on one half of the frozen representations and evaluate them on the other, where the baseline AI/human non-sarcastic false-positive rates are 84.8%/32.8% and source AUC is 0.982. LEACE, a linear concept-erasure method applied without retraining the sarcasm head, reduces linearly decodable source information: a source probe retrained after erasure gives AUC 0.589, while the rates change to 68.7%/49.5%. Their unweighted mean is nearly unchanged (58.8% to 59.1%), indicating error reallocation rather than overall improvement. Simpler source-direction and INLP removals produce no comparable reduction.

Separately, removing srcORTH-PC1---the maximum-variance direction after projecting out the estimated linear source direction---changes the rates to 66.7%/54.1%, while source AUC remains 0.982; random-direction controls produce no comparable change. We therefore treat srcORTH-PC1 as behaviorally relevant but semantically uncharacterized, not as an identified source feature.

#### 5.3.3 Evaluation on Qwen3.5-Generated Texts without Detector Retraining

We keep the trained detectors fixed and compare AI test subsets generated by DeepSeek-R1-0528-Qwen3-8B and Qwen3.5-9B (Ollama tag `qwen3.5:9b`). After quality control, the DeepSeek-generated and Qwen3.5-generated non-sarcastic subsets contain 1,513 and 1,550 outputs. On Qwen3.5 outputs, the plain joint and sarcasm-only false-positive rates are 91.3% and 7.9%, an observed gap of 83.4 pp compared with 67.0 pp on DeepSeek. The joint RCNN and interaction-projection rates are 22.1% and 29.5%. The higher joint-versus-sarcasm-only rate therefore persists on another generator, while absolute rates and gaps differ.
