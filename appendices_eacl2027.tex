...
\end{equation}
which retains the mean component. The task-head, single-probe, INLP, and Euclidean random-direction and random-subspace controls use the full-set pooled mean; LEACE, the covariance-whitened random controls, and srcORTH-PC1 use the fitting-half mean. INLP iteratively fits a source probe, normalizes its weight, orthogonalizes it against previously found directions by Gram--Schmidt, and deflates the residual representation. We evaluate $k\in\{1,2,4,8,16\}$ removed directions. Same-dimensional random-subspace controls orthogonalize Gaussian matrices by QR decomposition; both these controls and the permuted-label controls use \texttt{default\_rng(0)}.
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}p{2.25cm}cccc@{}}
    ...
    \bottomrule
  \end{tabular}
  \caption{Intervention summary. H$\times$n = human $\times$ non-sarcastic; AI$\times$n = AI $\times$ non-sarcastic; FPR in \%; gaps are computed from unrounded FPRs. Values for the covariance-whitened random controls are means over ten sampled directions; their source-group gaps range from 51.4 to 52.7 pp. AUC is the AUC of the source probe after erasure. Bold marks the two focal interventions discussed in \Sref{sec:mechanism}.}
  \label{tab:intervention}
\end{table}

The MGTD-head, source-probe, Euclidean-random, and INLP interventions do not materially narrow the source-group gap. LEACE reduces measured linear source decodability and the gap defined above from 52.0 to 19.1. Its gap change is not matched by any of the ten directions sampled in covariance-whitened space.
