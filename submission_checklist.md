...
**文件**：`manuscript_eacl2027.tex` + `appendices_eacl2027.tex` + `custom.bib` + `figures/`。**编译**：`xelatex → bibtex → xelatex → xelatex`（中文需 XeLaTeX）。输出 `manuscript_eacl2027.pdf` 共 **18 页**（07-15 主字体修复后由 19 页缩短；Times 更紧凑，正文结束提前，References 从 p.8 起、附录 p.11 起）。
...
**格式合规（已过）**：正文 §1–Conclusion 在 8 页内、Limitations 从 p.8 起、References p.9、附录 p.11；A4、字体全嵌入（含 Noto CJK）；`[review]` 模式（行号+页码+匿名）；两栏；Limitations/Ethical considerations 不编号、Conclusion 后 References 前；附录双栏、字母编号 A–E；无未定义引用/重复标签/overfull。**tex 摘要 149 词**——tex 摘要是比 md 更短的独立版本，`manuscript_eacl2027_en.md` 摘要 200 词，两者有意不同、勿互相对齐。
...
- **C（实验）**：报告参数量（§5.1）、超参与选择依据（§4 + 附录 B，类别权重与采样比先验设定、未在测试集调参）、统计描述（五次运行均值±std；Wilcoxon+BH-FDR、Wilson/McNemar，附录 C）、包版本（附录 B：torch 2.9.0/transformers 4.57.1/peft 0.18.0/RTX 3090 ✅）。
...
**标签口径（ⓔ 2026-07-11 + 07-14）**：AI 侧 `is_sarcastic` = 生成指令**构造定义**（claim-critical 句"双标签在构造上成立"保留）；全文保留**误判率/FPR**、四象限**不改名**；单头 19.3% 反事实 = 标签质疑的护甲。07-14 已在两 md + tex 加 caveat"instruction-conditioned diagnostic labels rather than independently annotated"（摘要/§3/Limitations）——**勿写回"独立人工标注"、勿删 caveat、勿删构造定义句**。2026-06 封存令维持（人类侧标签异构性不讨论）。标签改名 target_sarcastic_style / "讽刺预测率"= 已否决。
...
**术语（英译/全文）**：plain dual-head / single-head（禁 naive）；task-branch **interaction projection**（禁 cross-attention / cross-projection；B5 单向量 K/V 时 softmax≡1 退化为投影+残差）；**authorship**（AI-authorship detection / authorship head / authorship F1；禁 authenticity/veracity；"类人度/human-likeness"仅 §6 盲评维度）；**class-conditional failure**（禁 OOD failure；§2.3 引 Mueller"分布外"是文献描述、可留）；**跨生成器复现**（禁 cross-family；deepseek-r1:8b 是 Qwen 基座非 Llama）。
