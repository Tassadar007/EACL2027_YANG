...
我们分析产生表 3 中 86.3% 误报率的另行训练最终朴素联合模型的冻结表征。五折来源 probe 在联合与配对讽刺单任务表征上的 AUC 分别为 0.986 和 0.974，说明来源可解码性本身不能解释差距。在目标子群中，MGTD 头 AI logit 与讽刺 logit 相关（$\rho=0.519$）。MGTD 头与讽刺头方向均定义为两个类别权重向量之差；二者在普通欧氏余弦下近乎正交（$\cos\approx-0.03$），协方差归一化后的对齐度则为 0.49，相应的 probe 权重—讽刺方向对齐度为 0.41。这些结果属于相关性证据。

每项干预均在一半冻结表征上拟合、在另一半上评估；该评估半的基线 AI/人类非讽刺误报率为 84.8%/32.8%，来源 AUC 为 0.982。LEACE 是不重训讽刺头的线性概念擦除方法；它降低线性可解码的来源信息，使擦除后重训的来源 probe AUC 为 0.589，误报率变为 68.7%/49.5%。两组未加权均值几乎不变（58.8%→59.1%），说明这是错误重新分配而非整体改善。更简单的来源方向与 INLP 擦除没有产生相近降幅。

另行移除 srcORTH-PC1——即投影掉估计线性来源方向后方差最大的方向——会使误报率变为 66.7%/54.1%，而来源 AUC 仍为 0.982；随机方向对照没有产生相近变化。因此，本文将 srcORTH-PC1 解释为与行为有关但语义尚未确定的方向，而非已经识别出的来源特征。
