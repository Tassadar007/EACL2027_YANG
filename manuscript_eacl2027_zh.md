...
表中加粗仅标示两项诊断性干预（LEACE 与 srcORTH-PC1），不表示性能最优。解读：单方向与低维 INLP 变化很小；LEACE 降低测得的线性来源可解码性，并将来源特定 gap 从 52 缩至 19，十个抽样随机方向没有相近的 gap 变化。擦除一个与估计线性来源方向正交的高方差方向，在测得的线性来源可解码性不变时，把 AI 侧误判改为 66.7、gap 改为 12.6。这些对照表明预测对一个非随机高方差方向敏感，并把该方向的语义内容和因果作用确定为进一步分析的具体目标。在 LEACE 下，两侧非讽刺误判率的未加权均值几乎不变（58.8 → 59.1），表明相关方向会影响错误在人类文本与 AI 文本之间的分配。
