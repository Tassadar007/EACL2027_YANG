...
分析分为四个阶段：系统选择、联合—单任务配对比较、跨语言和生成器的来源 × 讽刺评估，以及表征级诊断。

### 5.1 系统级比较

**主干选择。** 在相同协议下，我们比较双头 mBERT（Devlin et al. 2019）、XLM-RoBERTa-base（Conneau et al. 2020）及 DeBERTa-v3 base/large（He et al. 2023），并对 DeBERTa-v3-large 测试交互投影和 RCNN 路径。DualTest 上报告两个任务的正类 F1。
...
DeBERTa 配置的平均 F1 高于 mBERT 与 XLM-R，RCNN 在两个任务上最高（讽刺 0.6309；MGTD 0.9301）。后续采用 DeBERTa-v3-large，以朴素编码器为参照，交互投影和 RCNN 为替代路径。完整结果见附录 C。
...
**排序与分数上移。** AI 文本内部的讽刺 AUC 仍为 0.82–0.88。朴素配对的概率中位数从 0.204 升至 0.767，且联合模型四分位区间高于决策阈值。高 FPR 因此来自分数上移，而非 AI 内部区分能力崩溃。
...
分数上移但 AI 内部排序仍保留，因此我们分析汇聚表征，即同时输入两个任务头的单个编码器向量。主要对象是最终朴素联合模型；配对讽刺单任务模型仅用于来源编码比较。

**来源可解码性。** 线性 probe 是在冻结表征上训练的分类器，用于检验某项属性能否在不改变原模型的情况下恢复（Hewitt and Liang, 2019）。逻辑回归来源 probe 在联合模型和讽刺单任务模型上的 AUC 分别为 0.986 和 0.974。两个模型都强烈编码来源，因此可解码性本身不能解释不同的子群差距。

**与讽刺决策的关联。** Probe 恢复来源不代表训练后的讽刺头遵循同一结构。Logit 是转化为概率或类别之前的类别分数；在 AI 生成非讽刺文本上，MGTD AI 与讽刺 logit 的相关为 $\rho=0.519$。二分类头的决策方向是类别 1 减类别 0 的权重向量。两个任务头在普通余弦下近乎正交（$\cos\approx-0.03$），但该几何不考虑表征实际变化区域。基于 Ledoit–Wolf 协方差的对齐按观测表征几何加权方向（Ledoit and Wolf, 2004）；任务头之间的绝对对齐度为 0.49，来源 probe 与讽刺头之间为 0.41。来源相关分数和数据加权方向因此跟随受影响决策。

**干预协议与直接来源方向。** 为超越关联分析，我们移除选定表征成分，再应用保持不变的讽刺头（Elazar et al., 2021）。任务头方向直接取自模型；数据驱动方向和变换在 DualTest 一半样本上拟合、另一半上评估。来源组差距定义为 AI 生成减人类非讽刺文本 FPR，来源 AUC 为干预后 probe 的 AUC。未经修改的评估半基线为 52.0 pp、来源 AUC 0.982；不同于前述 67.0 pp，该数字在单一模型内部比较来源组。

我们先移除 MGTD 头和来源 probe 方向。迭代零空间投影（INLP）反复学习线性来源方向并移除其联合张成空间（Ravfogel et al., 2020）；随机方向和子空间控制任意删除维度的影响。这些干预（包括最多 16 个 INLP 方向）均未实质缩小差距（51.9–72.1 pp）；协方差白化随机方向也无相近变化。直接或拟合的线性来源方向因此不能单独解释差距。

**协方差感知与残余几何。** 线性概念擦除方法 LEACE 以闭式协方差感知变换移除可线性访问的概念信息，同时尽量减少其他失真（Belrose et al., 2023）。LEACE 将差距从 52.0 缩至 19.1 pp，来源 AUC 从 0.982 降至 0.589；两来源组平均 FPR 几乎不变，说明错误被重新分配而非平均错误改变。

我们构造探索性、针对本研究的诊断 srcORTH-PC1，以检验拟合来源子空间之外的高方差几何。拟合三个 INLP 来源方向后，取其正交补中方差最大的方向；正式评估只从原始表征移除该方向。差距缩至 12.6 pp，而来源 AUC 在所报精度下仍为 0.982。残余来源 AUC 与 FPR 差距使用不同的读出：前者来自新拟合的来源 probe，后者来自冻结的讽刺头。两者的分离与以下解释一致：来源仍可从其他表征结构中恢复，而被删除方向会使两个来源组相对于固定讽刺决策边界发生不同变化。该结果表明模型对这一残余方向敏感；其语义内容仍需进一步表征与针对性因果检验。
...
表中加粗仅标示两项诊断性干预（LEACE 与 srcORTH-PC1），不表示性能最优。解读：单方向与低维 INLP 变化很小；LEACE 降低测得的线性来源可解码性，并将两来源组的误判率差从 52 缩至 19，十个抽样随机方向没有产生相近变化。擦除一个与估计线性来源方向正交的高方差方向，在测得的线性来源可解码性不变时，把 AI 侧误判改为 66.7、两来源组差值改为 12.6。残余来源 AUC 来自新拟合的 probe，而 FPR 差距来自冻结的讽刺头；两者的分离与来源仍可从其他结构恢复、但被删除方向使两个来源组相对固定讽刺边界发生不同变化的解释一致。该结果表明模型对这一残余方向敏感，并将其语义内容确定为进一步分析目标。在 LEACE 下，两侧非讽刺误判率的未加权均值几乎不变（58.8 → 59.1），表明相关方向会影响错误在人类文本与 AI 文本之间的分配。
