...
我们采用五次按折匹配的运行。第 $k$ 次运行使用 SarcTrain 的四折训练讽刺任务，以第五折验证；五次运行共用 AuthTrain 的固定 90/10 划分，其中约 2,000 条留出样本构成统一的 MGTD 验证集。每次运行重新载入相应配置的预训练编码器，并重新初始化其余模块与任务头；所有配置使用相同的折划分和种子方案。DualTest 仅用于最终评估。主表以五次运行的均值 ± 标准差概括讽刺折与初始化带来的变化；对于另行训练的最终模型，子群比率的 Wilson 区间概括 DualTest 样本层面的不确定性（表 3 与附录 C.7）。
...
我们先用五次运行汇总结果在 DualTest 上比较主干与联合—单任务流程，再使用另行训练的最终模型进行来源 × 讽刺分析、表征诊断、配对检验与 Wilson 区间估计。

### 5.1 主干对比

我们在同一协议下比较双头 mBERT（Devlin et al. 2019）、XLM-RoBERTa-base（Conneau et al. 2020）及 DeBERTa-v3 base/large（He et al. 2023）；对 DeBERTa-v3-large 另评估交互投影与 RCNN 配置。DualTest 上分别报告讽刺类和人类作者类的正类 F1。
...
在该实验方案下，DeBERTa 配置的平均 F1 高于 mBERT 与 XLM-R，RCNN 的均值最高：讽刺检测为 0.6309，MGTD 为 0.9301（表 1）。这些系统级比较共同反映主干类型、模型规模、表征设计和优化方案。基于这一实验结果，后续分析采用 DeBERTa-v3-large：以 plain shared encoder 为参照，并将 interaction-projection 与 RCNN 作为替代表征路径。宏平均、人类子集与分语言结果见附录 C。
...
每个讽刺行按 §4.2 的控制条件比较配对的联合与讽刺单任务流程；该对比评估包含任务调度和共享参数更新在内的完整联合训练流程，而非单独的 MGTD 监督。在十次按折匹配的讽刺比较中——每种配置五次——联合训练每次都降低讽刺 F1：朴素配置平均下降 6.08 pp，RCNN 下降 4.43 pp。MGTD 没有出现相应的一致下降，其平均 F1 分别变化 +1.67 和 +0.49 pp。RCNN 的联合—单任务讽刺差距较小。下文进一步定位这些总体退化集中在哪些来源 × 讽刺子群。

### 5.3 来源 × 讽刺子群分析

为判断总体退化是否均匀分布，我们分别评估四个来源 × 讽刺子群。每组只含一个讽刺参考类别，因此讽刺组报告召回率，非讽刺组报告误报率；人类文本保留数据集标注，AI 文本标签表示提示指定的风格。

#### 5.3.1 三种配置与仅讽刺对照的四象限结果

朴素配置与 RCNN 提供同配置配对比较：每对模型使用相同的纯人类讽刺数据，只有联合模型额外接受 MGTD 训练。交互投影配置没有匹配的讽刺单任务对照，因此仅作描述性比较。
...
末列方括号为测试样本上的 Wilson 95% 置信区间；全部象限的区间、配对检验与分数分布见附录 C.7。

![图 2](figures/fig1_quadrant_failure.png)

**图 2：AI × 非讽刺子集的讽刺误判率，五种检测器 × 两个生成器（误差线为 Wilson 95% 置信区间；数字与表 3 及附录 C.7 同源）。**

在按非讽刺条件生成的 AI 文本上，朴素联合与讽刺单任务误报率为 86.3% 和 19.3%，相差 67.0 pp；配对 RCNN 为 34.1% 和 19.3%，相差 14.8 pp。两组精确配对 McNemar 检验均有 $p<.001$（附录 C.8）。

在 AI 生成文本内部，表 3 各模型的讽刺 AUC 仍为 0.82–0.88，说明讽刺与非讽刺文本的相对排序大体保留。朴素配对的讽刺概率中位数在联合训练下为 0.767，讽刺单任务下为 0.204。结合误报率，这一模式与分数上移一致，而非来源内部排序崩溃。

朴素配置在非讽刺人类文本上的误报率变化为 4.3 pp，在 AI 文本上则为 67.0 pp（差值按未四舍五入比率计算）。这一方向在两种语言中均成立：朴素配置的联合减讽刺单任务差值为英文 +63.5 pp、中文 +79.4 pp，RCNN 为 +16.9 pp 和 +7.7 pp。语言仍与语料、体裁和标注设置混杂。总体退化因此集中于一个来源 × 讽刺子群，而非均匀分布于 DualTest。

#### 5.3.2 表征级诊断

**范围。** 我们分析产生表 3 中 86.3% 误报率的最终朴素联合模型的冻结汇聚表征；配对讽刺单任务模型仅用于来源 probe 对照。分析检验来源可解码性、其与讽刺决策的对齐以及表征干预的作用。结果只刻画这一个训练模型，不自动推广到其他配置或种子。

**可解码性与方向对齐。** 五折逻辑回归来源 probe 在联合表征上的 AUC 为 0.986，在配对讽刺单任务表征上为 0.974。两者均很高，但子群差距明显不同，因此可解码性本身不能解释联合模型错误。在 AI 非讽刺文本上，MGTD AI logit 与讽刺 logit 的相关为 $\rho=0.519$。任务头方向定义为类别权重向量之差，probe 方向为逻辑回归权重向量。MGTD—讽刺方向的普通欧氏余弦约为 $-0.03$；在观测表征的协方差结构下评估时，两个任务头间的对齐度为 0.49，来源 probe 与讽刺头间为 0.41。协方差使用汇聚后的无标签 DualTest 表征，因此该对齐是传导式的；以上结果均为相关性证据。

**干预协议。** 我们把 DualTest 样本分成互不重叠的拟合半与评估半。方向和变换只在拟合半估计，再应用于评估半；编码器与讽刺头保持冻结，剩余来源 AUC 由评估半内重新交叉验证的 probe 测量。未经干预时，AI 非讽刺文本的误报率为 84.8%，人类非讽刺文本为 32.8%（差距 52.0 pp），而完整 DualTest 上的 AI 侧比率为 86.3%；来源 AUC 为 0.982。

**来源信息擦除。** LEACE 将来源 AUC 降至 0.589，并把 AI 与人类误报率分别改为 68.7% 和 49.5%（差距 19.1 pp）。两组未加权均值几乎不变（58.8%→59.1%），说明主要是错误重新分配，而非平均改善。直接删除 MGTD 或 probe 方向、INLP 及随机方向对照均未产生相近的差距缩小。

**高方差敏感性检验。** 我们先从拟合表征中投影掉一个估计线性来源方向，再提取剩余空间中的最大方差方向 srcORTH-PC1。删除该方向上的投影后，AI 与人类评估半误报率分别变为 66.7% 和 54.1%（差距 12.6 pp），来源 AUC 仍为 0.982。因此，改变错误分配并不要求降低测得的线性来源可解码性。与一个估计方向正交也不排除其他来源信息，故 srcORTH-PC1 只能视为行为相关但语义未知的方向，而非已识别来源特征或完整因果解释。
...
使用相同的语言 × 风格模板，我们将未经重训的检测器用于 Qwen3.5-9B（Ollama 标签 `qwen3.5:9b`）输出。在 Qwen3.5 生成的非讽刺文本上，朴素联合与讽刺单任务误报率为 91.3% 和 7.9%（差距 83.4 pp；DeepSeek 上为 67.0 pp），配对 RCNN 为 22.1% 和 10.6%。因此，较高的联合训练误报率在另一生成器上仍然出现，但幅度不同。
