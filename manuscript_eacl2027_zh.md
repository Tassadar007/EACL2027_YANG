...
检测器分数在下游同样重要，因为它们可能被用作生成奖励。例如，ViSP 使用讽刺检测器分数构造 PPO 奖励（Wang et al. 2026；§2.4），这引出讽刺概率是否对应来源盲化 LLM 讽刺评分的问题。第二个任务头产生另一项独立问题：人类作者概率本身并不等于感知类人度。因此，§6 也比较这两个信号。尽管如此，现有工作仍大多分别研究讽刺检测与 MGTD，将二者统一为联合多任务问题的研究仍然缺失（检索协议见 §2.2）。

为了检验这些风险，而不是预设 MGTD 必然是有益的辅助任务，本文在双语数据上比较同配置联合流程与讽刺单任务流程，并构建 DualTest 开展来源 × 讽刺子群分析。所评估的联合训练流程大幅提高了按非讽刺条件生成的 AI 生成文本上的讽刺误报率。本文将这种集中于一个来源 × 讽刺子群、而非均匀分布于 DualTest 的退化称为类条件负迁移。该方向在 Qwen3.5-9B 生成的测试文本上仍然出现，两种替代表征配置的观测误报率低于朴素配置。通过结合线性 probe、任务头相关性和表征干预，本文表明来源相关特征与讽刺决策相互对齐，而且移除一个从数据中得到的高方差方向会改变子群差距。该方向编码了什么以及为何影响预测，是后续因果分析的具体目标。
...
- **表征证据与后续因果分析目标（§5.3.2）。** 来源信息在讽刺表征中高度可解码并与决策相关；概念擦除与一个基于数据所得高方差方向的对照表明，移除该方向会改变子群错误。该方向的语义内容和因果作用仍是进一步分析的具体目标。
...
其中 $\mathrm{CE}_w$ 为使用类别权重向量 $w$ 的讽刺交叉熵（权重见附录 B）。梯度流可由上式直接读出：讽刺批次上 $\partial\mathcal{L}/\partial W_a = 0$（MGTD 批次对称地有 $\partial\mathcal{L}/\partial W_s = 0$），而 $\theta_{\mathrm{sh}}$ 在两类批次上均接收梯度。共享编码器与中间层由两个任务共同塑造，每个分类头只由本任务数据更新。
...
训练与评价包含五次基于折划分的运行，随后统一在 DualTest 上评测。SarcTrain 预切为 5 折：第 $k$ 次运行以其中 4 折训练讽刺任务，留出的 1 折作讽刺验证集、提供 $\mathcal{L}_{s}^{(e)}$；AuthTrain 固定切分为 90% 训练 / 10% 验证并在五次运行中共用同一划分，验证部分提供 $\mathcal{L}_{a}^{(e)}$；AuthTrain 不做折轮换，使运行间差异集中于讽刺训练数据的变化。每次运行从同一预训练基座重新初始化并独立训练；全部架构与消融实验共用同一折划分与种子方案，跨模型比较按划分配对。五个模型训练完成后统一在 DualTest 上评测两个任务；DualTest 不进入讽刺或 MGTD 模型的训练与验证划分。主表报告五次运行的均值 ± 标准差，该标准差反映训练划分与初始化带来的运行间波动；测试集采样的不确定性由最终模型的置信区间单独量化（表 3 与附录 C.7）。
...
完成讽刺与 MGTD 模型训练后，我们首先使用 DualTest 比较编码器主干与中间配置，随后比较配对的双头与单头流程，最后按来源和讽刺标签将预测划分为四组。整体指标为五次运行均值 ± 标准差；子群指标来自另行训练的最终模型，模型比较在同一批文本上配对进行。

### 5.1 主干对比

我们在四个编码器主干上比较双头模型：mBERT（Devlin et al. 2019）、XLM-RoBERTa-base（Conneau et al. 2020），以及 DeBERTa-v3 base 与 large（He et al. 2023）；并在 large 编码器上加入 §4.1 的两个中间层变体。表中"讽刺 F1"为讽刺类（正类）F1，遵循 iSarcasmEval 的官方口径；"MGTD F1"为人类类（正类）F1。F1 在 DualTest 全集上计算，聚合中英两种语言与人类/AI 两个来源组；AI 生成文本的讽刺标签由生成指令构造（§3）。
...
表 1 报告五次运行均值 ± 标准差。在所评估的设置下，DeBERTa 配置的平均 F1 高于 mBERT 与 XLM-R。RCNN 的均值最高，讽刺 F1 为 0.6309，MGTD F1 为 0.9301。在五次运行中，两个中间层变体的观测标准差也比朴素 large 模型低 34–47%。由于该表同时改变主干类型、参数规模和中间表征，它不能分离其中任何一项设计选择的作用。对于另行训练的最终 RCNN 模型，中文和英文讽刺 F1 分别为 0.819 和 0.429；类别占比、体裁、标注程序及语言都会影响这一差异，因此不能把它解释为纯语言效应。宏平均、人类子集与分语言结果见附录 C。
...
表中每行比较联合模型与所列任务对应的单任务对照。对于讽刺行，配对的联合模型与仅讽刺模型共享编码器、中间配置、讽刺数据、折划分与讽刺任务超参；联合模型额外接收 AuthTrain 批次与 MGTD 更新。仅讽刺模型每轮遍历完整的讽刺训练折，联合模型则在 2:1 任务混合调度下抽取讽刺批次；比较仍按折配对。联合训练使朴素模型和 RCNN 的平均讽刺 F1 分别下降 6.08 和 4.43 个百分点，而 MGTD F1 平均只变化 +1.67 和 +0.49 个百分点。十个按划分配对的比较中——每种配置五个——联合模型的讽刺 F1 全部更低，差值为 −0.8 至 −8.6 个百分点。因此，持续出现的退化集中在讽刺检测，而不是两个任务同时变差。RCNN 的联合—仅讽刺差距较小，这一结果进一步指向容量与计算量匹配的实验，以检验哪些 RCNN 组件贡献了该差异。

### 5.3 来源 × 讽刺子群分析

整体 F1 可能掩盖只集中在某类文本上的失效。我们按来源和讽刺标签将 DualTest 分为四组：人类文本使用数据集标注，AI 生成文本的讽刺标签反映提示指定的风格，而非人类标注者的独立判断。由于每组只含一个参考类别，讽刺文本报告召回率，非讽刺文本报告误报率。

#### 5.3.1 三种配置与仅讽刺对照的四象限结果

我们在同一 DualTest 上比较五个检测器。三种联合配置用于观察子群错误是否随中间结构而变化；朴素配置和 RCNN 配置的讽刺单任务模型提供同配置对照。每一对模型使用相同的纯人类讽刺训练数据，只有联合模型额外接受 MGTD 训练。因此，配对差异衡量的是完整联合训练流程。
...
末列方括号为对测试样本的 Wilson 95% 置信区间；全部象限的区间、同批文本上的配对检验及各象限的分数分布见附录 C.7；这些分数模式与广泛上移一致，而非仅由阈值选择造成。

![图 2](figures/fig1_quadrant_failure.png)

**图 2：AI × 非讽刺子集的讽刺误判率，五种检测器 × 两个生成器（误差线为 Wilson 95% 置信区间；数字与表 3 及附录 C.7 同源）。**

朴素联合模型将 86.3% 的 AI × 非讽刺样本预测为讽刺；联合交互投影配置和联合 RCNN 配置的误报率分别为 40.9% 和 34.1%，而两个配对的讽刺单任务对照均为 19.3%。在同一批文本中，有 1,021 条只被朴素联合模型误判，只有 7 条只被其配对的讽刺单任务对照误判（$p<0.001$）。因此，大幅上升是在加入 MGTD 训练后出现的。朴素模型和 RCNN 提供配对对照；交互投影配置同时改变了架构与优化因素，因此只作为补充比较。

MGTD 头没有出现同等量级的错误：朴素模型和 RCNN 在该子集上的误报率分别为 6.1% 和 6.6%。在 AI 生成文本内部，各模型的讽刺 AUC 仍为 0.82–0.88，说明讽刺与非讽刺 AI 生成文本的相对排序大体保留。对于朴素配对，联合训练下的讽刺概率中位数为 0.767（IQR 0.635–0.839），讽刺单任务下为 0.204，因此联合分布的大部分位于固定的 0.5 阈值之上。该模式与广泛的分数上移一致，而不是来源内部排序丧失。本文将这一具体表现称为类条件负迁移。

这一变化方向在两种语言中均成立：在按非讽刺条件生成的 AI 文本上，联合减讽刺单任务的误报率差值在朴素配置中为英文 +63.5 pp、中文 +79.4 pp，在 RCNN 中分别为 +16.9 pp 和 +7.7 pp。由于语言仍与语料、体裁和标注设置混杂，这些分层结果只支持方向一致性，不用于隔离语言效应。

#### 5.3.2 表征级诊断

我们检验三个问题：来源是否只是在表征中线性可用，来源相关分数是否与讽刺决策对齐，以及表征干预是否改变子群差距。分析对象是产生表 3 中 86.3% 误报率的另行训练最终朴素联合模型的冻结表征；因此这些诊断不自动推广到其他配置或随机种子。

我们在冻结表征上训练五折交叉验证逻辑回归 probe，以预测文本来自人类还是 AI。联合模型与配对讽刺单任务模型的来源 AUC 分别为 0.986 和 0.974，说明来源可解码性本身不能解释差距。在目标子群中，MGTD 头 AI logit 与讽刺 logit 相关（$\rho=0.519$）。MGTD 头与讽刺头方向均定义为两个类别权重向量之差；二者在普通欧氏余弦下近乎正交（$\cos\approx-0.03$），协方差归一化后的对齐度则为 0.49，相应的 probe 权重—讽刺方向对齐度为 0.41。协方差归一化考虑观测表征实际变化的方向，而普通余弦只在欧氏参数空间比较分类器向量。这些结果属于相关性证据。

每项干预均在一半冻结表征上拟合、在另一半上评估；该评估半的基线 AI/人类非讽刺误报率为 84.8%/32.8%，来源 AUC 为 0.982。LEACE 是不重训讽刺头的线性概念擦除方法；它降低线性可解码的来源信息，使擦除后重新训练的来源 probe AUC 为 0.589，误报率变为 68.7%/49.5%。两组未加权均值几乎不变（58.8%→59.1%），说明这是错误重新分配而非整体改善；AI–人类误报率差距则由 52.0 pp 降至 19.1 pp。更简单的来源方向与 INLP 擦除没有产生相近降幅。

作为独立的敏感性检验，我们移除 srcORTH-PC1——即投影掉估计线性来源方向后方差最大的方向——以检验不直接删除该来源方向时是否也会改变错误分配。误报率变为 66.7%/54.1%，差距降至 12.6 pp，而重新训练的来源 probe AUC 仍为 0.982；随机方向对照没有产生相近变化。因此，改变子群错误分配并不要求降低线性来源可解码性。本文将 srcORTH-PC1 解释为与行为有关但语义尚未确定的方向，而非已经识别出的来源特征。

**表 4：表征级诊断结果（AI × 非讽刺子集，最终模型；协议见附录 E）**
...
| 来源方向正交第一主成分（srcORTH-PC1；投影掉估计线性来源方向后的最大方差方向）| 来源 AUC；误判率 | 保持 0.982；84.8% → 66.7% |
| 抽样随机方向 / 置换标签对照 | 误判率 | 无相近变化 |

#### 5.3.3 不重训检测器的 Qwen3.5 生成文本评估

我们保持训练好的检测器不变，比较由 DeepSeek-R1-0528-Qwen3-8B 与 Qwen3.5-9B（Ollama 标签 `qwen3.5:9b`）分别生成的 AI 测试子集。质量控制后，DeepSeek 与 Qwen3.5 生成的非讽刺条件子集分别保留 1,513 和 1,550 条。在 Qwen3.5 输出上，朴素联合与讽刺单任务误报率分别为 91.3% 和 7.9%，观测差距为 83.4 pp；DeepSeek 上的对应差距为 67.0 pp。配对 RCNN 的联合与讽刺单任务误报率分别为 22.1% 和 10.6%，联合交互投影配置为 29.5%。因此，联合流程相对于讽刺单任务流程的较高误报率在另一生成器上仍然出现，但绝对比率与差距大小不同。
...
**C.3 盲评协议、评审间一致性与逐组检验（§6）。** 每条样本以单条提示独立调用三个评审 API（2026 年 5 月；temperature = 0.3，GPT-5-nano 不支持温度参数、用其默认解码），提示仅含输入内容与被评文本、不含真实来源信息，并要求评审自行猜测来源后给出讽刺度与类人度两个 1–5 分评分；base 与 SFT 输出以相同提示分别评分。评审间逐条一致性为中等偏低：讽刺维度两两 Spearman ρ 为 0.20–0.38，类人维度为 0.03–0.23，因此逐条排序不被视为共同金标准。条件级观察另由六组语言 × 评审比较中一致的评分方向支持。正文使用三评审均分作为描述性汇总；由于一致性有限，逐评审结果仍不可替代。
...
**C.4 SFT 与人类参考的配对检验（补充）。** 该检验在生成评测集中文子集上进行（n = 180；GPT-5-nano 缺 3 条为 177）。这些比较提供了分评审、分维度的校准背景：DeepSeek-V3 在类人度维度上给 SFT 的评分显著低于参考（−0.161，p = 0.0025），而 GPT-5-nano 在讽刺维度上给 SFT 的评分显著高于参考（+0.186，p = 0.0114）。其余四组在所报阈值下未达显著。观测到的参考差异随评审和维度变化；这些检验未估计评审 × 维度交互。

**C.5 朴素检测器对照。** 打分：中文讽刺 0.765 → 0.763（−0.001，无变化）、英文 0.843 → 0.659（−0.184）；MGTD 中文 +0.760、英文 +0.705。相关性：总体讽刺 ρ = +0.142 / MGTD −0.214；中文讽刺 −0.037（不显著）/ MGTD −0.209；英文讽刺 +0.342 / MGTD −0.283。两种检测器均显示人类作者概率上升。中文讽刺概率的条件级变化对检测器敏感，而英文在两个检测器下均下降。
...
**C.7 测试实例带来的不确定性：置信区间、配对检验与子集 F1。** 本节针对每个另行训练的最终模型，以 Wilson 95% 区间量化象限比率、以 10,000 次逐样本 bootstrap 的 95% 分位区间量化 F1。这些区间不覆盖训练、生成器采样、语料构造或人类—AI 配对的不确定性，并与表 1、表 2 的运行间标准差互补。最终模型在 Qwen3.5 生成文本上复现了联合流程与讽刺单任务流程的差距方向，但幅度不同。
...
RCNN 的 MGTD F1 为：全集 0.938〔0.931, 0.945〕、中文 0.961〔0.952, 0.969〕、英文 0.923〔0.912, 0.933〕。由于人类子集中不存在 AI 标签的负例，不单独报告该子集的来源类别 F1；表 C.2 直接报告人类召回率。RCNN 中文讽刺 F1 高于英文（0.819 vs 0.429），但该差异同时与类别占比、体裁、标注程序及语言混杂：中文讽刺占 63.8%，英文为 14.2%，人类侧也来自不同基准。因此，两种正类 F1 不能解读为纯语言对比。RCNN 整体讽刺 F1 为 0.663，人类子集为 0.629，差异来自整体指标还包含 AI 生成文本；人类子集上联合流程与讽刺单任务流程的差异在 0.03 以内且方向不一，可见负迁移损失集中在包含 AI 生成文本的象限。

误判率按 0.5 阈值计。逐样本分数在该阈值附近呈现广泛差异：朴素双头在 AI × 非讽刺子集上的讽刺概率中位数为 0.767（四分位距 0.635–0.839），即分布中间 50% 均位于 0.5 之上；单头模型的中位数为 0.204（朴素）/ 0.069（RCNN）。另一项独立证据是，各模型在 AI 生成文本内部的 AUC 相近（0.82–0.88），说明内部排序基本保留。
...
**E.1 设置。** 全部分析在冻结模型（产出正文 86.3% 的朴素双头）上进行，不更新参数。样本被分为两半：拟合半用于估计方向，评估半用于计算指标；因此，本附录在评估半上的基线误判率（84.8%）与正文全集数字（86.3%）略异。代码自检表明：手算 logit 与模型输出逐元素一致（最大误差 1.4×10⁻⁶）；擦除投影后的残差约为 10⁻⁶；LEACE 后拟合半样本上的类均值差相对残余为 7×10⁻⁶，在同一半样本上评估的线性 probe AUC 为 0.514；真标签与置换标签下的 probe AUC 分别为 0.986 和 0.512。

**E.2 协议。** 各检验的原理与引用见 §5.3.2，此处记录实现细节。线性 probe 为 5 折交叉验证的逻辑回归，选择性对照以置换来源标签重训同一 probe；相关分析按文本长度三分位分层复核。对齐分析中，$\Sigma$ 是在全部 DualTest pooled 表征上拟合的 Ledoit–Wolf 协方差矩阵，特征值下限截断为 $10^{-8}$；每个 $w$ 表示任务头权重差或 probe 方向。对于待比较的两个方向 $w_1,w_2$，对齐度为 $\cos(\Sigma^{1/2}w_1,\Sigma^{1/2}w_2)$。擦除干预含单方向、移除方向数 $k=1$–$16$ 的 INLP 子空间与 LEACE；对照为十个抽样随机白化方向及置换标签重估方向后擦除。srcORTH-PC1 取对所估计线性来源方向正交化后的最大方差方向；正交于一个估计方向并不意味着独立于所有多维或非线性来源信息。
...
表中加粗仅标示两项诊断性干预（LEACE 与 srcORTH-PC1），不表示性能最优。解读：单方向与低维 INLP 变化很小；LEACE 降低测得的线性来源可解码性，并将两来源组的误判率差从 52 缩至 19，十个抽样随机方向没有产生相近变化。擦除一个与估计线性来源方向正交的高方差方向，在测得的线性来源可解码性不变时，把 AI 侧误判改为 66.7、两来源组差值改为 12.6。这些对照表明移除该方向会改变来源组间的错误分布，并把该方向的语义内容和因果作用确定为进一步分析的具体目标。在 LEACE 下，两侧非讽刺误判率的未加权均值几乎不变（58.8 → 59.1），表明相关方向会影响错误在人类文本与 AI 文本之间的分配。
