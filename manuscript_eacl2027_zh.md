...
MGTD 头没有出现同等量级的错误：朴素模型和 RCNN 在该子集上的误报率分别为 6.1% 和 6.6%。在 AI 生成文本内部，各模型的讽刺 AUC 仍为 0.82–0.88，说明讽刺与非讽刺 AI 生成文本的相对排序大体保留。联合训练改变的主要是分数水平：它把 AI 生成文本上的讽刺概率整体推高，使许多按非讽刺条件生成的文本越过固定的 0.5 阈值，而其内部排序仍大体保留。本文将这一具体表现称为类条件负迁移。

#### 5.3.2 表征级诊断

接下来我们检验，共享表征中的来源信息是否与较高的讽刺分数有关。共享编码器和中间层会接收 MGTD 更新，而讽刺头只使用纯人类的 SarcTrain 训练，因此它可能把来源相关变化误当作讽刺证据。这只是待检验的解释，不是已经确立的因果机制。

我们在冻结的朴素联合模型上开展诊断，完整协议见附录 E。线性 probe 可以从联合表征中预测文本来源，AUC 为 0.986；配对的仅讽刺表征也达到 0.974，说明来源可解码性是两个模型共同具有的背景属性。在联合模型中，AI × 非讽刺文本得到的 MGTD 头 AI logit 越高，其讽刺 logit 通常也越高（Spearman 0.52）。在白化后的表征空间中，由 MGTD 头和 probe 估计的来源方向也与讽刺方向呈中等程度对齐（$|\cos|\approx0.41$–0.49）。logit 相关与方向对齐共同提供了相关性证据，将来源相关结构与受影响的讽刺决策联系起来。

随后，我们修改冻结表征并重新计算预测。LEACE 将线性来源可解码性从 AUC 0.982 降至 0.589，并使 AI/人类非讽刺误报率由 84.8%/32.8% 变为 68.7%/49.5%。两组平均值几乎不变，说明 LEACE 主要在来源组之间重新分配错误。作为另一个关键对照，本文将投影掉估计线性来源方向后剩余的最大方差方向定义为来源方向正交第一主成分（srcORTH-PC1）。移除 srcORTH-PC1 后，AI/人类误报率变为 66.7%/54.1%，测得的来源可解码性仍为 0.982；十个随机方向没有产生相近变化。此前的相关性和方向对齐把来源相关结构与受影响的决策联系起来；干预则另行表明子群错误对 srcORTH-PC1 敏感，并将其确定为下一步因果分析的具体目标。
...
我们保持训练好的检测器不变，比较由 DeepSeek-R1-0528-Qwen3-8B 与 Qwen3.5-9B（Ollama 标签 `qwen3.5:9b`）分别生成的 AI 测试子集。质量控制后，DeepSeek 与 Qwen3.5 生成的非讽刺条件子集分别保留 1,513 和 1,550 条。朴素联合流程的误报率分别为 86.3% 和 91.3%，配对讽刺单任务流程分别为 19.3% 和 7.9%，差距由 67.0 pp 增至 83.4 pp。在 Qwen3.5 生成的文本上，联合 RCNN 与交互投影配置仍低于朴素联合流程（22.1% 与 29.5%）。因此，联合流程相对于讽刺单任务流程的较高误报率在另一生成器输出上仍然出现，但绝对比率与差距大小有所变化。
...
误判率按 0.5 阈值计，逐样本分数分布显示失效并非阈值伪影：朴素双头在 AI × 非讽刺子集上的讽刺概率中位数为 0.767（四分位距 0.635–0.839），整个分布右移越过阈值；单头模型的中位数为 0.204（朴素）/ 0.069（RCNN）。另一项独立证据是，各模型在 AI 生成文本内部的 AUC 相近（0.82–0.88），说明内部排序基本保留。
