...
这种耦合也出现在生成侧：检测器分数会被用作评价或奖励信号（如 ViSP，Wang et al. 2026；§2.4），而判断生成的讽刺是否像人类所写本身就涉及两个维度。然而，现有工作大多分别研究讽刺检测与 MGTD；将二者统一为联合多任务问题的研究仍然缺失（检索协议见 §2.2）。
...
对于朴素模型和 RCNN 的双头—单头比较，该协议固定架构、讽刺监督及讽刺任务专属设置，并将 MGTD 批次、任务调度、共享参数更新与联合早停视为一个完整流程。
...
基于 LLM 的数据增广要求生成样本能够传达讽刺，同时避免来源线索成为捷径。本文结果揭示了这一风险：相较于同架构单头对照，朴素双头模型更常将按非讽刺条件生成的 AI 文本预测为讽刺，使其误报率提高 67.0 个百分点。该方向在两种语言和第二个生成器上均保持一致；RCNN 与交互投影的误报率较低。表征干预显示预测对来源相关高方差结构敏感，但主要重新分配错误，尚未隔离出具体原因。人类作者概率与感知类人度也出现分歧，因此检测器分数在作为质量或奖励信号前，需要接受外部验证和来源 × 类别分层评估。
